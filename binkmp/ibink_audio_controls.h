...
#ifndef BINK_MP_IBINK_AUDIO_CONTROLS_H_
#define BINK_MP_IBINK_AUDIO_CONTROLS_H_
...
struct AudioVolume {
  /**
   * @brief Specifies the new volume setting from 0 (silent) to 32768 (normal)
   * to 65536 (maximum amplification).  Note that most sound systems don't do
...
struct AudioBalance {
  /**
   * @brief Balance, from 0 (all the way left) to 32768 (split evenly between
   * left and right), and 65536 (all the way right).
   */
  uint16_t value;
};
...
struct BINK_ATTRIBUTE_NO_VTABLE IBinkAudioControls {
   ...
   * @return Tracks count.
   */
  virtual uint32_t GetTracksCount() const noexcept = 0;
   ...
   * @return Track if any.
   */
  virtual std::unique_ptr<IBinkAudioTrack> GetTrackById(
...
#endif  // !BINK_MP_IBINK_AUDIO_CONTROLS_H_